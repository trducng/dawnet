...
def _merge_yaml_and_cli_args(yaml_args: Namespace, cli_args: list[str]) -> DictConfig:
    ...
    be specified with the parent field directly, e.g., model=tmodels.lora_llama2_7b
    instead of model._component_=models.lora_llama2_7b. Nested fields within the
    ...
    Example:
        ...
        >>>     a: 1
        >>>     b:
        >>>       _component_: models.my_model
        >>>       c: 3

        >>> python full_finetune --config config.yaml b=models.other_model b.c=4
        >>> yaml_args, cli_args = parser.parse_known_args()
        >>> conf = _merge_yaml_and_cli_args(yaml_args, cli_args)
        >>> print(conf)
        >>> {"a": 1, "b": {"_component_": "models.other_model", "c": 4}}
