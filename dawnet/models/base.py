...
def lstm_flops_counter_hook(module, input_layer, output_layer):
    """Calculate the number of flops in a lstm model

    This hook counts everything: the multplications, the additions, the
    activation,...
    
    # Arguments
        module [torch.nn.LSTM]: the LSTM module
        input_layer [tuple]: list of input layers
        output_layer [tuple]: list of output layers0
    """
    input_layer = input_layer[0]
    seq_length = input_layer.shape[0]
    batch_size = input_layer.shape[1]
    ...
    if module.bias:
        each_gate_count += 2 * hidden_size
    each_gate_count += 3* hidden_size       # sigmoid activation function
    gate_count = each_gate_count * 4        # there are 4 gates
    gate_count += 2 * hidden_size           # overhead for the tanh function

    # calculate the cell update operations
    cell_update_count = hidden_size * 3

    # calculate the hidden state update operations
    hidden_update_count = hidden_size * 6
