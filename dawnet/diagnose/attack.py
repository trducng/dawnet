...
def changing_input_view_feature_channel(
    ...
    def show_images(
        ...
        image_new = augment_image(
            ...
            pt=pt,
            elas_alpha=elas_alpha,
            elas_sigma=elas_sigma,
            blur_type=blur_type,
        ...
        if label is None:
            ...
            _ = view_string_prediction(prediction, label, to_print=True, notebook=True)

        if next(model.parameters()).is_cuda:
            image_torch = torch.FloatTensor(image_new, device=torch.device("cuda:0"))
            image_torch = image_torch.unsqueeze(0).unsqueeze(0).cuda()
        else:
        ...
        feature_map = run_partial_model(
            model, layer, image_torch, preprocess=preprocess
        )
    ...
    if layer_idx is not None:
        layer_slider = widgets.IntSlider(
            min=0, max=10, step=1, value=0, layout=Layout(display="none")
        )
        if channel_idx is None:
            feature_map = run_partial_model(
                model, layer_idx, image_torch, preprocess=preprocess
            )
            max_page = feature_map.shape[1] // 15
        ...
        if channel_idx is None:
            feature_map = run_partial_model(
                model, 0, image_torch, preprocess=preprocess
            )
            max_page = feature_map.shape[1] // 15
            page_slider = widgets.IntSlider(
                min=0,
    ...
    interact_manual(
        ...
        elas_sigma=widgets.FloatSlider(
            ...
            step=0.05,
            value=0.4,
            description="Elastic (sigma)",
            layout=activate("elas_sigma"),
        ),
        blur_type=blur_type,
        blur_value=blur_value,
...
def compare_model_response(
    ...
    if next(model.parameters()).is_cuda:
        input1_torch = torch.FloatTensor(input1, device=torch.device("cuda:0"))
        input1_torch = input1_torch.unsqueeze(0).unsqueeze(0).cuda()
        input2_torch = torch.FloatTensor(input2, device=torch.device("cuda:0"))
    ...
    def show_images(layer, page, crop):
        ...
        # retrieve the feature map
        layer = layer if layer_idx is None else layer_idx
        feature1_map = run_partial_model(
            model, layer, input1_torch, preprocess=preprocess
        )
        feature1_map = feature1_map.squeeze().cpu().data.numpy()
        feature1_map = feature1_map[:, :, left:right]
        feature2_map = run_partial_model(
            model, layer, input2_torch, preprocess=preprocess
        )
        ...
        # case where viewing only 1 channel in a layer
            ...
            image_diff = normalize_to_range(image1 - image2, min_value=0, max_value=255)

            min_value = min(np.min(image1), np.min(image2))
            max_value = max(np.max(image1), np.max(image2))

            image1 = normalize_to_range(
                image1, min_value=0, max_value=255, current_range=(min_value, max_value)
            )
            image2 = normalize_to_range(
                image2, min_value=0, max_value=255, current_range=(min_value, max_value)
            )
    ...
    def layer_update(*args):
        """Change the number of channels as the layer changes"""
        feature_map = run_partial_model(model, layer_slider.value, input1_torch)
    ...
    if layer_idx is not None:
        ...
        feature_map = run_partial_model(
            model, layer_idx, input1_torch, preprocess=preprocess
        )
        ...
        feature_map = run_partial_model(model, 0, input1_torch, preprocess=preprocess)
        crop_widget = widgets.Text(
            value="0,{}".format(feature_map.shape[-1]),
            placeholder="0,{}".format(feature_map.shape[-1]),
            description="Crop (left,right):",
    ...
    interact_manual(show_images, layer=layer_slider, page=page_slider, crop=crop_widget)
