...
from dawnet import config
import contextlib


def filter_submodules(module, criteria):
  """Finds sub-modules that satisfy some criteria.

    Args:
        module (torch.nn.Module): The module to search.
        criteria (callable): A function that takes a module and returns True if
          it satisfies the criteria.

    Returns:
        dict: A dictionary of sub-modules that satisfy the criteria. The keys are
          the module names and the values are the module objects.
    """
  found_modules = {}
  for name, submodule in module.named_modules():
    if criteria(name, submodule):
      found_modules[name] = submodule

  return found_modules


def suggested_lora(module):
  """Suggested model to become Lora"""
  linear = filter_submodules(
      module,
      criteria=lambda n, m: isinstance(m, nn.Linear) and n.
      startswith("trf_blocks")
  )
  return linear
...
class LoRALinear(nn.Module):
  def __init__(self, linear: nn.Linear, rank: int, alpha: float):
    ...
    self.lora_down = nn.Linear(
        in_features=linear.in_features, out_features=rank, bias=False
    )
    self.lora_up = nn.Linear(
        in_features=rank, out_features=linear.out_features, bias=False
    )

  def forward(self, x: torch.Tensor):
    base_out = self.base(x)
    proj = self.lora_down(x)
    lora_up = (self.alpha / self.rank) * self.lora_up(proj)
    return base_out + lora_up

  def lora_named_parameters(self, prefix="", lora: bool = True):
    if lora:
      yield from self.lora_up.named_parameters(prefix=f"{prefix}.lora_down")
      yield from self.lora_down.named_parameters(prefix=f"{prefix}.lora_down")
    else:
      yield from self.base.named_parameters(prefix=f"{prefix}.base")


def replace_linear_to_lora_linear(
    module: nn.Module, subs: dict, rank: int, alpha: float
) -> nn.Module:
  """Replace a linear module with its Lora variant

  Args:
    module: the nn.Module
    subs: a dictionary contain linear layers inside `module` that should be
      replaced with `LoRALinear`
    rank: lora rank
    alpha: lora alpha
  """
  for name in subs.keys():
    ...
    parent.set_submodule(
        parts[-1], LoRALinear(subs[name], rank=rank, alpha=alpha), strict=True
    )
  return module


def lora_named_parameters(model: nn.Module, lora: bool = True):
  if lora:
    for name, module in model.named_modules():
      if isinstance(module, LoRALinear):
        yield from module.lora_named_parameters(prefix=name)
  else:
    lora_params = []
    for name, module in model.named_modules():
      if isinstance(module, LoRALinear):
        for n, p in module.lora_named_parameters(prefix=name):
          lora_params.append(n)

    lora_params = set(lora_params)
...
class LoRASingleModel(FTRecipeInterface):
  ...
  This recipe is optimized for single GPU training. Training on CPU is not supported.
    ...
    """
  _supported_devices = ["cpu", "cuda", "mlx"]
  _supported_dist_methods = ["single", "distributed"]

  # Cons: having a cfg (DictConfig) make it more cumbersome to hack, though it
  # make the model easier to toggle. Though it essentially allows for a more
  # hierarchical organization of configs
  # Still allow kwargs
  # TODO: each configuration will have the appropriate setup, such that it works
  # during any phase of model training
  def __init__(self, cfg: DictConfig) -> None:
    # TODO: allow clone a new configuration, with updated value
    self._cfg = cfg
    ...
    self._save_adapter_weights_only = cfg.get(
        "save_adapter_weights_only", False
    )
    self._gradient_accumulation_steps = cfg.gradient_accumulation_steps
    self._clip_grad_norm = cfg.get("clip_grad_norm", None)

    self._enable_async_checkpointing = cfg.get(
    ...
    if self._enable_activation_offloading:
      ...
      if not self._enable_activation_checkpointing:
        ...
        self._enable_activation_checkpointing and
        cfg.checkpointer.model_type != "LLAMA3_VISION"
    ):
      utils.log_rank_zero(
          self._logger,
  ...
  def setup(self, cfg: DictConfig) -> None:
    ...
    # TODO: this is hard-code a specific type of model
    ...
    # setup model
    self._model = config.instantiate(cfg.model)
    self._adapter_config = cfg.lora

    suggested = suggested_lora(self._model)
    self._model = replace_linear_to_lora_linear(
        self._model, suggested, cfg.lora.lora_rank, cfg.lora.lora_alpha
    )
    self._model.to(torch.device("mps"))

    lora_params = {
        n: m
        for n, m in lora_named_parameters(self._model, lora=True)
    }
    set_trainable_params(self._model, lora_params)
    ...
    self._optimizer = self._setup_optimizer(
        ...
        opt_state_dict=(
            ...
            if training.OPT_KEY in checkpoint_dict else None
        ),
    )
    ...
    with self._profiler as prof:
      ...
      for curr_epoch in range(self.epochs_run, self.total_epochs):
        ...
        for idx, batch in enumerate(self._dataloader):
          ...
          if (
              curr_epoch == 0 and self.profiler_profile_memory and
              idx == self.profiler_wait_steps + self.profiler_warmup_steps and
          ...
          current_num_tokens = (batch["labels"]
                                != self._loss_fn.ignore_index).sum()
          ...
          if (idx + 1) % self._gradient_accumulation_steps == 0:
            ...
            if (
                curr_epoch == 0 and self.profiler_profile_memory and
                idx == self.profiler_wait_steps + self.profiler_warmup_steps +
                self.profiler_active_steps and self._device.type == "cuda"
            ):
            ...
            if (
                self.global_step % self.save_every_n_steps == 0 and
      ...
      self._logger.info(
          "Checkpoint saved in {:.2f} seconds.".
          format(time.perf_counter() - start_save_checkpoint)
      )
...
if __name__ == "__main__":
  ...
  recipe = LoRASingleModel(cfg=cfg)
  recipe.setup(cfg=cfg)
  recipe.train()
